...
import SingleBytes from './Shared/SingleBytes';
...
const Bytes: FC<PublicationProps> = ({ publication, comments }) => {
  if (!publication) {
    return <DefaultTags />;
  }
  ...
  return (
    <>
      <Tags
        ...
        url={`${BASE_URL}/bytes/${publication.id}`}
      />
      <header>
        <SingleBytes publication={publication} h1Content />
      </header>
      <div>
        {comments?.map((comment) => {
          ...
          return (
            <div key={id}>
              <SingleBytes publication={comment} />
            </div>
          );
        })}
      </div>
