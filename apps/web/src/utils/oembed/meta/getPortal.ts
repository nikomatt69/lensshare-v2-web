import type { ButtonType, Portal } from '@lensshare/types/misc';
import type { Document } from 'linkedom';

const getPortal = (document: Document): null | Portal => {
  const version = document.querySelector('meta[property="lensshare:portal"]');
  const image =
    document.querySelector('meta[property="lensshare:portal:image"]') ||
    document.querySelector('meta[property="og:image"]');

  let buttons: Portal['buttons'] = [];
  for (let i = 1; i < 5; i++) {
    const action = document.querySelector(
      `meta[property="lensshare:portal:button:${i}:action"]`
    );
    const button = document.querySelector(
      `meta[property="lensshare:portal:button:${i}"]`
    );
    const type = document.querySelector(
      `meta[property="lensshare:portal:button:${i}:type"]`
    );
